...
class DB_FSM_Reducer(Generic[T_STATE, T_CHAR]):
    ...
    def __init__(
        ...
        self.binding_score_space = binding_score_space
        self.run_id = run_id
        ...
        logger.info(
            f"Using {binding_score_space.__name__} for binding scores for state reduction of FSM for {run_id}"
        )
    ...
    def find_reduced_fsms(
        ...
        self._hc = HierarchicalClustering(self.binding_score_space)
        self._multisets: Multisets[T_STATE] = Multisets(
            self.binding_score_map, self._hc
        )
        ...
        while not self._merge_list.is_empty():
            ...
            reduced_fsm_idx += 1
            yield self._reduced_fsm, self._total_error, (
    ...
    def _validate_extracted_pair_with_minimal_distance(
        ...
        distances = [d for d, _ in self._mergeable_pairs_distances]
        min_dist = min(distances)
        pairs_with_min_dist = [
            pair
            for d_val, pair in self._mergeable_pairs_distances
            if np.isclose(d_val, min_dist)
        ]
        assert (v1, v2) in pairs_with_min_dist and np.isclose(d, min_dist), (
    ...
    def _validate_total_error(self):
        """total error == sum of distances"""
        origin_scores, current_scores = get_scores(
            self.binding_score_map, self._reduced_fsm_binding_score_map, self._f
        )
        calculated_error = np.sum(
            self._hc.space.distance(origin_scores, current_scores)
        )
        assert np.isclose(
            calculated_error, self._total_error
        ), f"err: {calculated_error} != {self._total_error}"
